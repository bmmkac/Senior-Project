    ...
    def filter_positive_classifications(self, image_list, center_list):
        ...
        for i, img in enumerate(image_list):
            if img.shape != (50,50,3):
                continue
            pre_processed = img.astype(np.float32)
            pre_processed = np.expand_dims(pre_processed, axis=0)

            pp_ims.append(pre_processed)
            pp_centers.append(center_list[i])
        #for i, img in enumerate(image_list):
            #if img.shape != (50,50,3):
            #    continue
        ...
        pp_ims = np.asarray(pp_ims)
        #print(pp_ims.shape)
        shape = pp_ims.shape
        pp_ims = np.reshape(pp_ims,(shape[0], 50, 50, 3))
        results = self.model.predict(pp_ims, batch_size=128)
        #print(results)
        for i, result in enumerate(results):
            if result < THRESHOLD:
                ecell_center_list.append(pp_centers[i])
